    ...
    def get_trakt(self, path, args, media_type, timeout):
        ...
        try:
            ...
            if '401' in str(error) or '400' in str(error) or '403' in str(error):
                ...
                if (config.conf.get('Trakt', 'oauth2_refresh')
                ...
                print("Error: You likely have a bad ClientID/Secret or expired/invalid token."
    ...
    def post_trakt(self, path, post_json, args, media_type, timeout):
        ...
        try:
            response = self.trakt_session.post(
                f"https://api.trakt.tv/users/{self.user}/{path}", headers=self.trakt_hdr,
                data=post_json, timeout=timeout if not args.timeout else self.post_timeout)
            response.raise_for_status()
            ...
            if '401' in str(error) or '403' in str(error):
                ...
                response = self.post_trakt(
                    path, post_json, args, media_type,
